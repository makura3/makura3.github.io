...
description: 初心者向けにVue.jsの導入方法からテンプレート構文、コンパイル(コンパイラとビルドの種類)についてまとめました。サンプルソースも用意しています。
category: Vue
id: vue-summary
tags:
  - Vue
...
文字列テンプレートや、単一ファイルコンポーネントがこれにあたります。
...
<br>
ということは、一番使用する頻度が高いであろう単一ファイルコンポーネントを使用した場合、完全ビルドが必要なのでコンパイルする時間もかかるし、ランタイム限定ビルドよりファイルサイズが大きい...そんなの嫌です。  
<br>
どうにかして単一ファイルコンポーネント＋ランタイム限定ビルドにすることはできないんでしょうか？  
ランタイム限定ビルドで動作させることができれば、読み込み時間の短縮が期待できそうです！  

### 事前にコンパイルする方法
なんと単一ファイルコンポーネント＋ランタイム限定ビルドにする方法はあります！  
<br>
それは、*「vue-loaderなどを使用しtemplateを事前にJavaScriptへコンパイルしておく」* です。  
<br>
事前にコンパイルしておけば、ランタイム限定ビルドで事足りるのです。なるほど。
